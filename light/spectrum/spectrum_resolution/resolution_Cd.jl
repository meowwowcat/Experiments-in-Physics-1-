...
df1 = CSV.read("light/spectrum/data/Cd 41ms.txt", DataFrame; types=Float64, header=false, delim='\t')
df2 = CSV.read("light/spectrum/data/Cd 41ms bg.txt", DataFrame; types=Float64, header=false, delim='\t')
...
#ピークをCSVファイルに保存
CSV.write("light/spectrum/spectrum_resolution/peak/Cd.csv", result)


peak = scatter!(ax, filtered_positions, filtered_heights, color=:red)
save("light/spectrum/spectrum_resolution/figure/Cd_peaks.png", fig)
...
# 書き込むファイルの初期化
rm("light/spectrum/spectrum_resolution/bunkainou/bunkainou_Cd.txt")  
...
for i = 1:n
    ...
    open("light/spectrum/spectrum_resolution/bunkainou/bunkainou_Cd.txt", "a") do io
        @printf(io, "Cd\n")
...
save("light/spectrum/spectrum_resolution/figure/Cd_fits.png", fig_2)